...
def fitreso_scan(
    ...
    with open(log_path, 'w') as fh:
        ...
        fh.write(f"# label   : scan point.  `pre` = raw mov PDB vs ref (no alignment,\n")
        fh.write(f"#           no bending); `hkl00` = mov aligned to ref, no shift field;\n")
        fh.write(f"#           `hklNN` = one canonical HKL admitted per row (NN = 01..10);\n")
        fh.write(f"#           `frXX` = progressive fit to XX Å; `best` = parabola-vertex\n")
        fh.write(f"#           d_opt fit (see best-row footer).\n")
        fh.write(f"# RMSD    : CA RMSD (Å) after the bent shift field is applied to mov.\n")
        fh.write(f"#           `pre` row uses raw mov PDB; other rows use post-alignment mov.\n")
        fh.write(f"# active  : number of canonical HKLs contributing to the shift-field fit\n")
        fh.write(f"#           at this row (grows as more HKLs are admitted progressively).\n")
        fh.write(f"# Rbent   : Rfac after F-space (k+B) scaling of bent → ref, computed on\n")
        fh.write(f"#           FWT amplitudes (map-quality metric — moves with the scan).\n")
        fh.write(f"# Rbend   : Rfac of bent vs hkl00 bent (how much the PSDVF moved density)\n")
        fh.write(f"# k, B    : F-space scale + isotropic B (Å²) used for Rbent.  With\n")
        fh.write(f"#           scale_target='fobs' (default), k+B is fit on raw Fobs and is\n")
        fh.write(f"#           scan-invariant (a crystal-to-crystal Wilson-B property).\n")
        ...
        fh.write(f"# train/  : cross-validation holdout metrics (only populated when\n")
        fh.write(f"# hold /    holdout_frac > 0 was passed to fitreso_scan).  All in\n")
        fh.write(f"# hprfit    Cartesian Å.  `train` = RMSD of predicted-vs-observed shifts\n")
        fh.write(f"#           over the *training* atoms (SVD-fit population); `hold` = same\n")
        fh.write(f"#           over the held-out atoms (honest CV number); `hprfit` = held-out\n")
        fh.write(f"#           atoms' pre-fit RMSD (baseline — how much they shifted before\n")
        fh.write(f"#           bending).  hold ≪ hprfit means the field genuinely predicted\n")
        fh.write(f"#           unseen atoms; hold ≈ train means well-determined, no over-fit.\n")
        fh.write(f"# peak    : most-extreme diff-map peak σ (sign preserved) in this row's\n")
        fh.write(f"#           diff_norm.map (z-scored diff map).\n")
        fh.write(f"# atom    : nearest atom to the peak.  `(r)` = from ref PDB, `(m)` = from\n")
        fh.write(f"#           mov PDB.  Format: chain/resnum-resname/atom-name(source).\n")
        fh.write(f"# dist    : peak-to-atom distance (Å).\n")
        ...
        fh.write(f"# t(s)    : wall time in seconds spent on this scan point (dominated by\n")
        fh.write(f"#           the SVD design-matrix solve for the progressive iteration count\n")
        fh.write(f"#           at that HKL admission count).\n\n")
